...
export class PermissionsStore {
  ...
  readonly items = signal<PermissionOutput[]>([]);
    ...
    this.permissionsService
      ...
      .subscribe({
        ...
        error: (apiError: ApiError) => {
          this.error.set(apiError.problem.detail ?? 'Erro ao carregar permissões.');
        }
      });
  }
      ...
      .subscribe({
        ...
        error: (apiError: ApiError) => {
          if (apiError.status === 400 && apiError.problem.errors) {
            ...
            this.error.set(apiError.problem.detail ?? 'Erro ao criar permissão.');
          }
        }
      });
  }
  ...
  remove(id: string): void {
    ...
    this.permissionsService
      ...
      .subscribe({
        next: () => this.load(),
        error: (apiError: ApiError) => {
          this.error.set(apiError.problem.detail ?? 'Erro ao excluir permissão.');
        }
      });
  }
}
